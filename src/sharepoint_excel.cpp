...
static bool LooksLikeZipArchive(const std::string &content) {
	return content.size() >= 4 && static_cast<unsigned char>(content[0]) == 0x50 &&
	       static_cast<unsigned char>(content[1]) == 0x4B && static_cast<unsigned char>(content[2]) == 0x03 &&
...
static SharePointFileInfo GetFileInfo(const std::string &url, const std::string &token) {
	...
	if (docs_pos == std::string::npos) {
		...
		if (!item_id_val || !drive_id_val) {
			...
			throw InvalidInputException("Could not resolve SharePoint sharing URL to a drive item. "
	...
	if (!drives_value || !duckdb_yyjson::yyjson_is_arr(drives_value) ||
	    duckdb_yyjson::yyjson_arr_size(drives_value) == 0) {
...
static void SharepointDownloadExcelScalar(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	for (idx_t i = 0; i < args.size(); i++) {
		...
		if (url.find(".xlsx") == std::string::npos && url.find(".xlsm") == std::string::npos &&
		    url.find(".XLSX") == std::string::npos && url.find(".XLSM") == std::string::npos) {
			throw InvalidInputException("File must be an Excel file (.xlsx or .xlsm). "
		...
		if (content.empty()) {
			throw IOException("Downloaded file is empty. SharePoint may have returned an empty redirect response or "
		...
		if (!LooksLikeZipArchive(content)) {
			...
			for (idx_t j = 0; j < preview_length; j++) {
				error << " " << std::hex << std::uppercase << std::setw(2) << std::setfill('0')
