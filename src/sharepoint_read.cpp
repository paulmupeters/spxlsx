...
static std::string GetListIdByName(
    ...
    std::ostringstream path;
    path << "/v1.0/sites/" << site_id << "/lists?$filter=" 
         << SharepointUtils::UrlEncode(filter.str());
    ...
    if (!lists_data.contains("value") || lists_data["value"].empty()) {
        throw InvalidInputException("List not found: '" + list_name + "'. Check that the list name is correct and you have access.");
    }
...
static unique_ptr<GlobalTableFunctionState> SharepointReadInit(
    ClientContext &context,
...
static void SharepointReadFunction(
    ...
    if (state.response_json.empty()) {
        ...
        state.response_json = CallGraphApiListItems(
            bind_data.site_id,
            bind_data.list_id,
            bind_data.token,
            "",  // select all fields
        ...
        std::cerr << "[DEBUG] List items response (first 500 chars): " << state.response_json.substr(0, 500) << std::endl;
        state.row_index = 0;
    }
    ...
    if (state.row_index >= items_count) {
        ...
        if (response.contains("@odata.nextLink")) {
            ...
            if (path_start != std::string::npos) {
                ...
                // Fetch next page
                state.response_json = PerformHttpsRequest(
                    "graph.microsoft.com",
                    next_path,
                    bind_data.token,
                    HttpMethod::GET
